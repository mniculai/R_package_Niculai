...
```
```
# It looks like the three graphs show that emergence of butterfly populations to be later as the temperature rises. the more the years rise the later in time and temperature that butterflies begin to emerge. 
```
...
```
looking at the data it seems only 1.5% show any relationship between year and the emergence day
```
...
```{r, eval=FALSE}
butterfly_species_count <- butterfly_data%>%
  count(SummerTemp, ButterflySpecies)
ggplot(data = butterfly_species_count, mapping = aes(x = SummerTemp, y = n)) +
    geom_line() +
...
```{r, eval=FALSE}
butterfly_species_count <- butterfly_data%>%
  count(SpringTemp, ButterflySpecies)
ggplot(data = butterfly_species_count, mapping = aes(x = SpringTemp, y = n)) +
    geom_line() +
    facet_wrap(facets = vars(ButterflySpecies))
```
...
```{r, eval=FALSE}
butterfly_species_count <- butterfly_data%>%
  count(Day, ButterflySpecies)
ggplot(data = butterfly_species_count, mapping = aes(x = Day, y = n)) +
    geom_line() +
    facet_wrap(facets = vars(ButterflySpecies))
```
...
```{r}
butterfly_species_count_merged <- butterfly_data_joined%>%
    count(Day, ButterflySpecies)

ggplot(data = butterfly_species_count_merged, mapping = aes(x = Day, y = n)) +
    geom_line() +
    facet_wrap(facets = vars(ButterflySpecies))
```
...
```{r}
#The relationship between temperature and day of emergence seems to be stretching to later in the days with colder temperatures seeing greater emergence of butterflies. Kharouba and Vellend's hypotheses speaks of long-term temporal changes as temperature fluctuates increase and viability of butterfly species can decline as emergence becomes stretched out longer and later in days. some species like papillo zalicaion seem to be more resilient in temperature fluctuations and due to this may be for resilient to change and viability long term but others do not fair the same and have short windows or low emergence because of global warming. 
```
